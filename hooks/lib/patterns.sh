...
# ask_for_access: asking the user for files/output the AI can read itself (incl. screenshot/visual).
...
ASK_FOR_ACCESS_DESCRIPTION="Asking the user to paste, share, send, upload, attach, or drop files/contents/logs/terminal output, or for screenshots/visual confirmation (paste a screenshot, I can't see, tell me what you see) that the AI can read directly with its own tools (Read, Bash, Grep, filesystem MCP, screen)."
ASK_FOR_ACCESS_CORRECTION="Do not ask for access you already have. Read the file, run the command, grep the logs, capture the screen yourself. Only ask the human for things outside the machine."

# emotional_labor: performing human emotional labor (enthusiasm, gratitude, apologies, filler).
EMOTIONAL_LABOR_REGEX="i'?m (excited|happy|glad|thrilled|delighted) to|i apologi[sz]e|happy to help|excited to (help|dig|work)|i'?d love to|thank you for your patience|sorry for the (delay|wait|confusion)|great question|hope this helps|let me know if you need|\\b(sure|certainly|of course)!"
...
HUMAN_CAPACITY_CORRECTION="Do not frame compute as human strain. 'Complex', 'tricky', and 'takes time' are not your constraints. Decompose, fan out, and execute now."
